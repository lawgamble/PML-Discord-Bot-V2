...
function pickupGame(filePath, message, arguments, command) {
  ...
  if(gameStarted && arguments[0] === "end") {
    gameStarted = stopPickupGame(message, ninetyMinuteTimer, thirtyFiveMinuteTimer, filePath, timer);
    return 
  } 
...
setTimeout(() => {
  const interval = setInterval(() => {
    let data = readAliasData(filePath);
    console.log(data.teams["RED Team"])
      gameEnded = checkIfGameEnded(data)
        checkInactivePlayers(filePath, data, message);
        if(!gameStarted || gameEnded === true) {
        wipeRedAndBlueTeams(message, filePath, thirtyFiveMinuteTimer, ninetyMinuteTimer, gameStarted)
